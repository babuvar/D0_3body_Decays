...
if job_type=='signal':
	...
	sig_mc_dict={	"D_pi0mumu" : 	{
					#"dirs" : ["D_pi0mumu_5K","Db_pi0mumu_5K"],
					"dirs" : ["D_pi0mumu_50K","Db_pi0mumu_50K"],
					"rec_flag" : 12
					}
	}
	...
	for mode in modes:
		for i in range(2):
			...
			os.system('bsub -q s -oo log/signal_mc/%s.log'%sig_mc_dict[mode]['dirs'][i] +' basf2 -l INFO reco_3mumu.py  %s' %sig_mc_dict[mode]['rec_flag'] + ' mc signal ' + '%s%s' %(mdst_loca,sig_mc_dict[mode]['dirs'][i]) +  ' root/signal_mc/%s.root' %sig_mc_dict[mode]['dirs'][i])
		...
		#for expNo in expNos:
			...
			for startRun, endRun in run_list[dataType][expNo].items():
				...
				for eventType in eventTypes:
					...
					os.system('bsub -q l -oo log/generic_mc/stream_%s/%s.log'%(streamNo,filename) +' basf2 -l INFO reco_3mumu.py  %s'%rec_flag + ' mc generic ' + ' %s '%expNo + ' %s '%startRun + ' %s '%endRun + ' %s '%eventType + ' %s '%dataType + ' %s '%belleLevel + ' %s '%streamNo + ' root/generic_mc/stream_%s/%s.root'%(streamNo,filename) )
