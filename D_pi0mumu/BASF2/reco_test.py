...
#variable dictionary
var_dict = json.load(open("variables_3mumu.json"))
...
ma.fillParticleList('mu+:all', '', path=reco_path)
...
cuts_Dst='0.1445 < DeltaM < 0.1465 and Dst_mom_st > 2.2 and 0.119 < pi0_mass < 0.151 and gam1_energy > 0.05 and gam2_energy > 0.05 and mu1_PID > 0.1 and mu2_PID > 0.1'
...
if int(split_flags[1]) == 1:
        ...
        #Rare D
        ma.reconstructDecay('D0:pi0mumu -> pi0:mdst  mu+:all mu-:all', cut=cuts_D, path=reco_path)
        ma.reconstructDecay('D*+:pi0mumu -> D0:pi0mumu pi+:all',cut=cuts_Dst, path=reco_path)
        ma.matchMCTruth('D*+:pi0mumu', path=reco_path)
        ...
        #ma.vertexTree('D*+:pi0mumu', ipConstraint=False, updateAllDaughters=False, conf_level=-2.0, path=reco_path)
        ma.variablesToNtuple('D*+:pi0mumu', tuple_vars, filename=outputBelle2ROOTFile, treename = 'tree1', path=reco_path)
